    ...
    try:
        ...
        sql = """
                SELECT media_id, play_count, progress, lastviewed, storage_location
                  FROM mediaserver.UserMediaConsumption FULL OUTER JOIN mediaserver.MediaItem USING (media_id)
                 WHERE username=%s
                   AND progress < 100.0
                   AND progress != 0;
        """
...
def get_song(song_id):
    ...
    try:
        ...
        #############################################################################
        sql = """SELECT song_title, string_agg(DISTINCT(Artist.artist_name),',') artists, length, string_agg(md_value , ',') AS Genre
                 FROM mediaserver.Song S JOIN mediaserver.Song_Artists SA ON (S.song_id = SA.song_id)
		                JOIN mediaserver.Artist ON (performing_artist_id = artist_id)
		                JOIN mediaserver.MediaItemMetadata ON (S.song_id = media_id)
		                JOIN mediaserver.Metadata USING (md_id)
                 WHERE S.song_id = %s
				 GROUP BY song_title, artist_name, length
        """
...
def get_song_metadata(song_id):
    ...
    try:
        ...
        #          FROM mediaserver.Song S JOIN mediaserver.Song_Artists SA ON (S.song_id = SA.song_id)
		#                 JOIN mediaserver.Artist A ON (performing_artist_id = artist_id)
		#                 JOIN mediaserver.ArtistMetaData AM ON (A.artist_id = AM.artist_id)
		#                 JOIN mediaserver.Metadata USING (md_id)
        #                 JOIN mediaserver.MetaDataType USING (md_type_id)
        #          WHERE S.song_id = %s
        # """

        # FROM Arthur: I think here we need a song's meta data rather than its artists'
        # and I think there must lack songs' meta data
        ...
        #TODO
        #FIXME
        # This works in pgadmin but i can't do it in python since it needs the same parameter 
    #    sql = """
    #              SELECT md_type_name, md_value,
				#(
...
def get_podcast(podcast_id):
    ...
    try:
        ...
        #############################################################################
        #DISTINCT md_value, DISTINCT md_type_name, md_type_id, md_id, media_id, podcast_id, podcast_title, podcast_uri, podcast_last_updated, podcast_episode_uri, podcast_episode_title, podcast_episode_published_date, podcast_episode_length
        # select *
        # from mediaserver.Podcast natural join mediaserver.PodcastEpisode
        #      natural join mediaserver.MediaItemMetaData
        #      natural join (SELECT DISTINCT md_value, DISTINCT md_type_name
        #                      FROM mediaserver.Metadata
        #                           natural join mediaserver.MetaDataType) a
        # where podcast_id = %s
        # """
...
def get_podcast_genres(podcast_id):
    """
    Get all genres for an podcast by the podcast ID in your media server
    """
    ...
    try:
        sql = """
        SELECT DISTINCT(md_value) as podcastgenres
        FROM mediaserver.Podcast natural join mediaserver.PodcastEpisode
             natural join mediaserver.MediaItemMetaData
             natural join mediaserver.Metadata
             natural join mediaserver.MetaDataType
        WHERE podcast_id = %s AND md_value = 'podcast genre'
        """
...
def get_all_podcasteps_for_podcast(podcast_id):
    ...
    try:
        ...
        # This query is working in pgAdmin, but nothing was displayed in the page
        sql = """
        select *
...
def get_podcastep(podcastep_id):
    ...
    try:
        ...
        sql = """
        select *
        from mediaserver.PodcastEpisode natural join mediaserver.MediaItemMetaData
        natural join mediaserver.MetaData
        natural join mediaserver.MetaDataType
        where media_id = %s
        """

        # Cannot check if it is correct before issues of query 7 solved
...
def get_album(album_id):
    ...
    try:
        ...
        sql = """
        select *
        from mediaserver.Album natural join mediaserver.AlbumMetaData
        natural join mediaserver.MetaData
        natural join mediaserver.MetaDataType
        where album_id = %s
        """
...
def get_album_genres(album_id):
    ...
    try:
        ...
        SELECT DISTINCT(md_value) as songgenres
          FROM mediaserver.Album NATURAL JOIN mediaserver.Album_Songs NATURAL JOIN mediaserver.Song 
               FULL OUTER JOIN mediaserver.MediaItemMetaData ON(Song.song_id = MediaItemMetaData.media_id) 
               NATURAL JOIN mediaserver.MetaData NATURAL JOIN mediaserver.MetaDataType
        WHERE album_id = %s;
        """

        #Karlo~ THIS NOW WORKS (we needed to call it 'songgenres' for it to pop up)
...
#####################################################
def find_matchingmovies(movie_title):
    """
    ...
    try:
        ...
        """
        #'LIKE lower(%s)' is too exact, '~ lower(%s)' provides suggestions, so its a bit nicer

        r = dictfetchall(cur,sql,(movie_title,))
        print("return val is:")
        print(r)
        cur.close()                     # Close the cursor
        conn.close()                    # Close the connection to the db
...
def find_matchingpodcasts(searchterm):
    ...
    try:
        # Try executing the SQL and get from the database
        sql = """
        select podcast_id, podcast_title, podcast_uri, podcast_last_updated, count(media_id) as count
        from mediaserver.Podcast natural join mediaserver.PodcastEpisode
...
#####################################################
#   Find all matching podcast episodes
#####################################################
def find_matchingpodcasteps(searchterm):
    """
    Get all the matching Podcast Episodes in your media server
    """
    ...
    try:
        # Try executing the SQL and get from the database
        sql = """
        select media_id, podcast_episode_title, podcast_episode_published_date, podcast_episode_length
        from mediaserver.PodcastEpisode
        where lower(podcast_episode_title) ~ lower(%s)
        order by media_id
        """
...
def add_song_to_db(location, songdescription, title, songlength, songgenre, artistid):
    """
    Add a new song in your media server
    """
    #########
    # TODO  #  
    #########
